...
accuracy_limit = 1e-6;
for t_end_index = 1:length(t_end)
    figure('name',['Implicit Euler with t_end ' num2str(t_end(t_end_index))]);
%     suplabel(['(N_x,N_y)=' num2str(N_xy)],'y');
    set(gcf,'renderer','painters');
    subplot_counter = 1;
    for cur_N_xy = N_xy
        for cur_dt = dt
            ...
            T_res = impl_euler(T_start,0,t_end(t_end_index),cur_dt,cur_N_xy,cur_N_xy,accuracy_limit);
...
%             plot_labeler(handle,cur_dt,cur_N_xy,cur_N_xy,left,top)
    ...
    suplabel(['Implicit Euler with t_{end}= ' num2str(t_end(t_end_index))],'t');
end

function y_t  = AM_Lin1(y0, d_t, t_end)
% Implements the Linearization 1 of Adams Moulton Method
...
for i = 2 : length(y_t)
	y_prv = y_t(i-1);
	G_y = @(y)(y - y_prv - (d_t/2)*(7*(1-y_prv/10)*y_prv + 7*(1-y/10)*y_prv));
	diff_G_y = @(y)(d_t*7*y_prv/20+1);
...
%figure('name',strcat('AM Lin1, dt: ', num2str(d_t)));			% JFT
